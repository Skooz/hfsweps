...
self.Penetrate 		= 15					// How deep Generic Default goes
self.Flightvector 	= self.Entity:GetUp()*((950*52.5)/66)	// Velocity in m/s, FIRST NUMMER is SPEED (FrameTime)
...
self.Drift		= 0.08					// How much the bullet will drift in flight (Inaccuracy)
...
function ENT:Think()
if self.Tracer==0 and self.Data.x==0 then		
self.Tracer=1
...
//Trail:SetKeyValue("rendercolor","200 200 200")
