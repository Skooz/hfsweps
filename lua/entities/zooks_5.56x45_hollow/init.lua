...
self.Penetrate 		= 3					// How deep Generic Default goes
...
self.TissueDamage	= math.Rand(30,40)			// Player damage is multiplied by 2 for some reason
self.BallisticDrag	= 50					// Fraction of velocity lost per tick, higher is less
self.Drift		= 0.15					// How much the bullet will drift in flight (Inaccuracy)
...
if hitgroup == HITGROUP_GENERIC 					then 	dmginfo:ScaleDamage( 1 ) 			elseif
   ...
   hitgroup == HITGROUP_HEAD 						then 	dmginfo:ScaleDamage( 5 ) 			else
	...
	// THIS IS BULLET RICOCHET	
		...
		if (Dot*math.Rand(0.04,1)*mats[tr.MatType][1])>-0.03 then		// If it doesnt hit head on,
